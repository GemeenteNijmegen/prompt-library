...
5. For each chat client the Organisation will deploy: create a confidential OAuth client in Keycloak with the gallery-audience scope, the appropriate scopes from the catalogue, the deployment's redirect URIs, and PKCE required. Instantiate [`keycloak/templates/org-deployed-client.json`](../../keycloak/templates/org-deployed-client.json) — a placeholder-templated block that gets the mechanical bits right every time (confidential, PKCE-S256, `gallery-defaults`/`prompt:read` defaults, and the documented default-vs-optional scope split with no `admin:*` / `prompt:moderate` / `prompt:publish:public`). It is a **consistency aid, not a security control** (see Revision 3 above — role-holding is the only real gate). Hand the `client_id` + `client_secret` to the Organisation's deployment team via a secure channel.
