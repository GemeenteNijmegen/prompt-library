...
| **Organisation Admin** | A designated person inside an Organisation with elevated gallery-side authority (draft visibility beyond their own, per `is_org_admin`). **v1: does not grant Keycloak admin-console access** — real per-Organisation user/scope self-service is deferred (see ADR 0003, ADR 0007); until then it's Gallery-Operator-mediated. | Keycloak `organization-admin` realm role — a gallery-facing flag only in v1, not yet scoped to one Organisation's admin-console access | A federated Entra End User with the additional Keycloak role |
